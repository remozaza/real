...
if (preg_match('/^(\/chk|\.chk|!chk)/', $text)) {
    ...
    if (!checkAccess($userid)) {
        $sent_message_id = send_reply($chatId, $message_id, $keyboard, "<b> @$username You're not PAID user❌</b>", $message_id);
        exit();
    }
...
$c4 = substr($cc, -4);
    ...
    if (empty($cc) || empty($cvv) || empty($mes) || empty($ano)) {
        bot('editMessageText', [
            ...
            'text' => "<b>♤•Wrong Format! ⚠️</b>%0A♤•USAGE: <code>/bu cc|mm|yy|cvv</code>%0A♤GATE-WAY: BRAINTREE AUTH%0A♤CREATED: [2024-03-23]",
            'parse_mode' => 'html',
